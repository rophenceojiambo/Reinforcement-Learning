...
```{r echo=FALSE}
bytes <- file.size("Reinforcement-Learning.Rmd")
words <- bytes/10
minutes <- words/200
```
...
ggplot(stocks_data, aes(x = date, y = close, color = symbol)) +
  geom_line(size=1) +
  labs(title = "Closing Price of Stocks", x = "Date", y = "Closing Price", color = "Stock Name") +
...
# Create a ggplot object with the closing price and moving averages plotted against the date, colored by the stock symbol
ggplot(stocks_data%>% filter(symbol=="NFLX"), aes(x = date)) +
  geom_line(aes(y = adjusted), color= "black", size = 1) +
  geom_line(aes(y = ma10), color = "darkgreen", size = 1) +
  geom_line(aes(y = ma20), color = "blue", size = 1) +
  geom_line(aes(y = ma50), color = "red", size = 1) +
  labs(title = "Closing Price and Moving Averages of Stocks",
       x = "Date",
       y = "Price",
       color = "Stock Name") +
  scale_x_date(date_labels = "%Y", date_breaks = "1 year")
