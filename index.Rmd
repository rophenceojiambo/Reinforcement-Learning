...
Our code is implementing the Q-learning algorithm to create a simple trading strategy for stocks. As the Q-learning algorithm is a model-free reinforcement learning technique, it is used to learn an optimal policy based on trial and error, thus involving building a Q-table that maps the current state and action to the expected future reward. In this case, the state is the current stock price, and the action is either to buy or hold/sell.
